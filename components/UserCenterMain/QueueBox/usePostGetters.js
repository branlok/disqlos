...
export default function usePostGetters() {
    ...
    const ownPostsResponse = useQuery("fetchQueuedPosts", () => db.collection("USERS").doc(userId).collection("PRIVATE_POSTS").orderBy("createdOn", "desc").limit(10).get().then(querySnapshot => {
        let docArray = [];
