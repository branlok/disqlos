  ...
  return (
    <div className="relative text-s font-gray-700 bg-custom-pink-550  dark:bg-cb-3  w-full flex flex-col justify-between items-center text-center rounded-md px-2  my-2 overflow-hidden">
      ...
      <div className="relative w-full flex-initial flex flex-col justify-center items-center pb-4 bg-white-100 border-gray-400 mt-4  ">
        ...
        <button onClick={() => setShowConfirmDelete(true)} className="text-xs text-red-1000 opacity-50 hover:opacity-100 transition font-bold dark:text-white">
          Delete
        </button>
      </div>
      {showConfirmDelete && <div className="absolute left-0 top-0 h-full w-full bg-gray-100 flex flex-col justify-center items-center text-black">
        <h1 className="text-lg font-bold">Confirm Delete?</h1>
        <div className="flex justify-center items-center">
          <button onClick={() => setShowConfirmDelete(false)} className=" dark:border-transparent rounded-md px-2 py-1 hover:bg-green-500 hover:text-white m-2">Return</button>
          <button onClick={() => deleteQueueMutation.mutate()} className="border dark:border-transparent rounded-md px-2 py-1 bg-gray-800 text-white hover:bg-red-500 hover:text-white m-2">Confirm</button>
        </div>
      </div>}
    </div>
  );
