...
function UserCard({ name, caption, imageURL, link, active }) {
  ...
  return (
    ...
    >
      <div
        className={`w-full h-full px-1 ${dynamicBg} rounded-sm flex flex-row items-center justify-center cursor-pointer`}
      >
        ...
        <div className="flex-initial px-2 flex flex-col justify-center border">
          ...
          <p className="text-xs text-gray border">Last posted {caption}</p>
        </div>
      </div>
    </div>
  );
